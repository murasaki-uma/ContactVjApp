...
void ofApp::update() {
	...
	noiseline.update();
	noiseline02.update();
	arline.update(float(scaledVol)*volLine);
	noisewalker.update();
	field.update(float(scaledVol)*volField);
...
void ofApp::drawFboTest() {
	...
	//    noiseline.draw();
	//    if(drawField) 
	if (drawAudioLine) arline.draw(drawNoReactLine);
	//    if()
...
void ofApp::drawFboTrailsTest() {
	...
	if (drawLineParticle02)noiseline02.draw();
...
void ofApp::draw() {
	...
	ofPushMatrix();
	ofTranslate(fboXposition, fboYposition);
	...
	if (fboMain)
	{
		...
		mainFbo.draw(0, 0, fboWH->x,fboWH->y);
	}
	...
	if (fboAlpha)
	{
		...
		rgbaFboFloat.draw(0, 0, fboWH->x, fboWH->y);
	}
...
void ofApp::setupGui() {
	...
	*/
	...
	parameters_fbo.add(fullScreen.set("fullscreen", false));
	parameters_fbo.add(fboWH.set("WidthHeihgt", ofVec2f(1920,1080/2), ofVec2f(0, 0), ofVec2f(1920*2, 1080)));
    ...
    parameters_fbo.add(fboXposition.set("X", 0, -ofGetWidth(), ofGetWidth()));
	...
	parameters_fbo.add(mainAlpha.set("main alpha", 255, 0, 255));
	parameters_fbo.add(alphaAlpha.set("alpha alpha", 255, 0, 255));
	...
	parameters_alpha.add(drawLineParticle02.set("drawLineParticle02", false));
	...
	animationSettings.add(drawNoReactLine.set("noreact line", false));

    animationSettings.add(volField.set("vol field", 0.5,0.0,20.0));
    animationSettings.add(volLine.set("vol line", 10.0,0.0,30.0));
